...
var pickRandomMeans = function(k) {
    ...
    for (var i=0; i<k; i++){
        x = Math.floor(Math.random()*640); //640 is the size of the canvas, want means to stay within this size
        y = Math.floor(Math.random()*480); //480 is the size of the canvas, want means to stay within this size
        means.push({x:x, y:y});
    }
    return means;
    //return format:
