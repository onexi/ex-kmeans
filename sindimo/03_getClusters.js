...
function getClusters(k){
    ...
    //Create an array of clusters, each cluster has an array of points belonging to that cluster
    for (var i=0; i<k; i++){

        var current_cluster_number = i;
        var current_cluster_array = [];

        //for every point, assign it to the cluster it belongs to
        for (var j=0; j<size; j++){

            var point = data[j];

            //Check if current point belongs to current cluster, and if so then add it
            if (point.centroid === current_cluster_number){
                current_cluster_array.push(point);
            }

        }

        //Save info for this current cluster
        clusters.push(current_cluster_array);
